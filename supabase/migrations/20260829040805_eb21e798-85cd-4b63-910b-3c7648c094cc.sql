...
drop policy if exists "slides_public_read" on public.hero_slides;
create policy "slides_public_read" on public.hero_slides for select using (true);
drop policy if exists "slides_admin_write" on public.hero_slides;
create policy "slides_admin_write" on public.hero_slides
  for all to authenticated using (public.is_admin()) with check (public.is_admin());
